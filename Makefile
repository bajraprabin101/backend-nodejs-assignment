...
start/db: ## Start database container
	...
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env up -d test-app-db test-app-adminer

.PHONY: stop/db
stop/db: ## Stop database container
	@echo "🛑 Stopping database (Docker)..."
	@$(DOCKER_COMPOSE) -f $(DOCKER_COMPOSE_FILE) --env-file .env stop test-app-db test-app-adminer
